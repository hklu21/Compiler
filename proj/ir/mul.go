...
func NewMul(target int, sourceReg int, operand int, opty OperandTy) *Mul {
	return &Mul{target, sourceReg, operand, opty}
}
...
func (instr *Mul) GetImmediate() *int {
	...
	//Mul instruction has two forms for the second operand: register, and immediate (constant)
...
func (instr *Mul) SetLabel(newLabel string) {
	// Mul does not work with labels can we can skip implementing this method.
}
func (instr *Mul) String() string {
	...
	out.WriteString(fmt.Sprintf("\tmul %s,%s,%s", targetReg, sourceReg, operand2))

	return out.String()

}

func (instr *Mul) TranslateToArm(frame *codegen.Frame) []string {
	...
	if !frame.IsArgument(instr.sourceReg) {
		...
		output = append(output, fmt.Sprintf("\t\tmul x%d,x%d, x%d",
