...
func NewMove(target int, sourceReg int, sourcety OperandTy) *Move {
	return &Move{target, sourceReg, sourcety}
}

func (instr *Move) GetTargets() []int {
	targets := make([]int, 0)
	targets = append(targets, instr.target)
	return targets
}
func (instr *Move) GetSources() []int {
	sources := make([]int, 0)
	sources = append(sources, instr.sourceReg)
	return sources
}
func (instr *Move) GetImmediate() *int {
...
func (instr *Move) String() string {
	...
	out.WriteString(fmt.Sprintf("\tmov %s,%s", targetReg, sourceReg))

	return out.String()

}

func (instr *Move) TranslateToArm(frame *codegen.Frame) []string {
