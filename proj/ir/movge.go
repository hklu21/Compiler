...
func NewMovge(target int, sourceReg int, sourcety OperandTy) *Movge {
	return &Movge{target, sourceReg, sourcety}
}

func (instr *Movge) GetTargets() []int {
	targets := make([]int, 0)
	targets = append(targets, instr.target)
	return targets
}
func (instr *Movge) GetSources() []int {
	sources := make([]int, 0)
	sources = append(sources, instr.sourceReg)
	return sources
}
func (instr *Movge) GetImmediate() *int {
...
func (instr *Movge) String() string {
	...
	out.WriteString(fmt.Sprintf("\tmovge %s,%s", targetReg, sourceReg))

	return out.String()

}

func (instr *Movge) TranslateToArm(frame *codegen.Frame) []string {
	output := make([]string, 0)
	if instr.sourceReg == instr.target {
		return output
	} else {
		if instr.sourcety == IMMEDIATE {
			output = append(output, fmt.Sprintf("\t\tmovge x%d,#%d",
			...
			output = append(output, fmt.Sprintf("\t\tmovge x%d,x%d",
				frame.LocalOffset, frame.LocalOffset+1))
		}
		output = append(output, fmt.Sprintf("\t\tstrdge x%d,[x29,#-%d]",
