package ir

import (
	"bytes"
	"fmt"
	"proj/codegen"
)

type Print struct {
	target int // The target register for the instruction
}

func NewPrint(target int) *Print {
	return &Print{target}
}

func (instr *Print) GetTargets() []int {
	targets := make([]int, 0)
	targets = append(targets, instr.target)
	return targets
}
func (instr *Print) GetSources() []int {
	sources := make([]int, 0)
	return sources
}
func (instr *Print) GetImmediate() *int {
	return nil
}
func (instr *Print) GetLabel() string {
	return ""
}
func (instr *Print) SetLabel(newLabel string) {
}
func (instr *Print) String() string {

	var out bytes.Buffer
	targetReg := fmt.Sprintf("r%v", instr.target)
	out.WriteString(fmt.Sprintf("\tprint %s", targetReg))
	return out.String()

}

func (instr *Print) TranslateToArm(frame *codegen.Frame) []string {
	output := make([]string, 0)
	output = append(output, fmt.Sprintf("\t\tldr x%d,[x29,#%d]",
		frame.LocalOffset+1, frame.FindVarOffset(instr.target)))
	output = append(output, fmt.Sprintf("\t\tadrp x%d, .PRINT",
		frame.LocalOffset+2))
	output = append(output, fmt.Sprintf("\t\tadd x%d, x%d, :lo12:.PRINT",
		frame.LocalOffset+2, frame.LocalOffset+2))
	output = append(output, fmt.Sprintf("\t\tmov x0, x%d",
		frame.LocalOffset+2))
	output = append(output, fmt.Sprintf("\t\tmov x1, x%d",
		frame.LocalOffset+1))
	output = append(output, fmt.Sprintf("\t\tbl printf"))
	return output
}
